...
document.addEventListener('DOMContentLoaded', () => {
  const selAno = $('f-ano');
  selAno.innerHTML = anosDisponiveis().map(a => `<option value="${a}">${a}</option>`).join('');
  // Exercício seguinte por padrão: é o que está em tramitação na CMO no
  // segundo semestre, que é quando a nota é pedida.
  estado.ano = anosDisponiveis()[0];
  selAno.value = estado.ano;

  $('f-tipo').addEventListener('change', ev => { estado.tipo = ev.target.value; carregar(); });
