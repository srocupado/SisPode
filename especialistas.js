...
const ESPECIALISTAS = [
  ...
  // ==========================================================
  {
    ...
    // O tema 62 (Ciência, Tecnologia e Inovação) foi RETIRADO daqui: é largo
    // demais e acionava a lente de IA para o PL 914/2024, que é descarbonização
    // automotiva. Cada lente falsa custa uma seção inteira de parecer. Os
    // gatilhos desta lente (LGPD, ANPD, deepfake, algoritmo, criptoativo) são
    // específicos o bastante para acionar sozinhos.
    temas: [37],
...
/**
 * Gatilhos de uma palavra só que MEDIDAMENTE acionam a lente errada.
 *
 * Não removi nenhum termo da lista da casa — o que uma palavra significa no
 * campo é decisão de vocês. O que este conjunto faz é marcar a lente como de
 * confiança BAIXA quando ela foi acionada só por um desses termos e por mais
 * nada, para o analista decidir antes de gastar uma seção inteira de parecer.
 *
 * Medidos em 04/09/2026:
 *   "tratamento"    → "tratamento de dados pessoais" aciona Saúde
 *   "incorporação"  → "incorporação de empresa" aciona Saúde
 *   "programa"      → qualquer "Programa Nacional de…" aciona Orçamentário
 *   "benefício"     → "benefício da dúvida" e afins acionam Orçamentário
 *   "contrato"      → aciona Consumidor em qualquer matéria administrativa
 *   "prescrição"    → prescrição penal × prescrição civil
 *   "preço"/"mercado" → acionam Regulatório em matéria de consumo
 */
const GATILHOS_AMBIGUOS = new Set([
  'tratamento', 'incorporação', 'programa', 'benefício', 'auxílio', 'contrato',
  'prescrição', 'preço', 'mercado', 'cria', 'institui', 'dispõe sobre', 'garantia',
  'crédito', 'licença', 'autorização', 'concessão', 'taxa', 'piso', 'teto',
]);
...
function sugerirEspecialistas({ temas = [], ementa = '', textoEmendas = '' } = {}) {
  ...
  for (const e of ESPECIALISTAS) {
    ...
    // Todos os gatilhos que casaram, e não só o primeiro: dois termos casando
    // é sinal muito mais forte que um, e é o que separa lente real de ruído.
    const casados = [...new Set([
      ...(e.gatilhos || []).filter(g => _casaGatilho(ementa, g)),
      ...(e.gatilhos || []).filter(g => _casaGatilho(textoEmendas, g)),
    ])];
    const naEmenta = (e.gatilhos || []).find(g => _casaGatilho(ementa, g));
    const noTexto = (e.gatilhos || []).find(g => _casaGatilho(textoEmendas, g));
    if (!e.sempre && !porTema.length && !casados.length) continue;

    // Confiança baixa: nenhum tema oficial, e o único gatilho que casou é um
    // termo que já se mediu acionando a lente errada.
    const soUmAmbiguo = !e.sempre && !porTema.length
      && casados.length === 1 && GATILHOS_AMBIGUOS.has(casados[0].toLowerCase());
    ...
    out.push({
      ...
      confianca: soUmAmbiguo ? 'baixa' : 'alta',
      ressalva: soUmAmbiguo
        ? `Acionada apenas por "${casados[0]}", termo que já se mediu acionando esta lente indevidamente. Confirme antes de incluir.`
        : null,
      motivo: motivos.join('; '),
