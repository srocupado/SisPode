...
- **Detecção de nota desatualizada** (sob demanda): o botão **"Verificar atualizações"** reconsulta a tramitação e marca com o badge **"⚠ Pode estar desatualizada"** as análises cujo **texto operativo** (PRLP/PRLE, SBT-A, SSP ou EMS) foi superado por um documento mais recente — comparando o que embasou a nota salva com a tramitação atual (por URL **e** data, para não dar falso positivo, ex.: PRLP anterior ao retorno do Senado). A checagem inclui EMS/SSP (cenários 5/6/7). O tooltip do badge indica qual documento novo apareceu; regerar a nota limpa o alerta
