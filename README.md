...
- **Detecção de nota desatualizada**: marca com o badge **"⚠ Pode estar desatualizada"** as análises cujo **texto operativo** (PRLP/PRLE, SBT-A, SSP ou EMS) foi superado por um documento mais recente na tramitação — comparando o que embasou a nota salva com a tramitação atual (por URL **e** data, para não dar falso positivo). Roda **automaticamente** no carregamento (com os dados já trazidos pelo enriquecimento) e pelo botão **"Verificar atualizações"**, que faz a checagem completa, incluindo EMS/SSP. O tooltip do badge indica qual documento novo apareceu; basta regerar a nota para limpá-lo
