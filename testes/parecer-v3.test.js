...
(async () => {
  ...
  console.log('== M7 relato de decisão × afirmação própria; G8 nível declarado pelo programa ==');
  {
    ...
    ok(gVoto.reprovacoes.some(r => r.gate === 'G10' && /NÃO recomenda e NÃO defende posição/.test(r.detalhe)), 'G10: recomendação de voto do próprio parecer manda refazer a redação');
    const frasesRelato = [
  ...
  console.log('== atores, implementação, aprimoramentos, viabilidade e conclusão ==');
  {
    ...
    const bruta = {
      ...
      conclusao: { id: 'CC', ponto_de_decisao: 'Decide-se instituir o marco nacional da negociação coletiva no setor público.', estabelecido: 'o texto altera os arts. 92 e 240 da Lei 8.112/1990', aberto: 'o impacto fiscal alegado pelos municípios não foi estimado', falta_para_decidir: 'a estimativa de impacto, que nenhum documento do processo traz', evidencias: ['P1', 'A1'] },
    };
    ...
    const comPosicao = T.validarTese({ ...JSON.parse(JSON.stringify(bruta)), conclusao: { id: 'CC', ponto_de_decisao: 'Recomenda-se aprovar o substitutivo com as emendas.', falta_para_decidir: 'x', evidencias: ['A1'] } }, cat, { nivel: 'C', emVigor: false });
    ok(!comPosicao.tese.conclusao && comPosicao.removidas.some(r => r.id === 'CC' && /toma posição ou orienta voto/.test(r.motivo)), 'conclusão que recomenda ou toma posição é removida — o parecer não defende posição');
    const semFalta = T.validarTese({ ...JSON.parse(JSON.stringify(bruta)), conclusao: { id: 'CC', ponto_de_decisao: 'Decide-se instituir o marco.', evidencias: ['A1'] } }, cat, { nivel: 'C', emVigor: false });
    ok(!semFalta.tese.conclusao && semFalta.removidas.some(r => r.id === 'CC' && /o que falta para decidir/.test(r.motivo)), 'conclusão sem "o que falta para decidir" é removida');
    ...
    ok(c2.tese.conclusao && /ignora o impacto/.test(c2.tese.conclusao.contestada) && /RESSALVA do contraditório/.test(uCC.texto) && /O que se decide:/.test(uCC.texto), 'conclusão contestada guarda a ressalva e a redação a recebe junto com o ponto de decisão');
    ...
    ok(/Nada de recomendação de voto nem de posição em NENHUMA parte/.test(pt) && /SÓ sinais objetivos/.test(pt), 'prompt: nada de recomendação em parte alguma; viabilidade só com sinal objetivo');
    ...
    ok(!gC.reprovacoes.some(r => r.gate === 'G10') && rC.itens.find(i => /^M11/.test(i.item)).ok, 'conclusão neutra passa: diz o que se decide e o que falta, sem recomendar');
    for (const frase of ['Recomenda-se a aprovação do texto [CC].', 'A melhor opção é aprovar o substitutivo [CC].', 'Sugerimos a rejeição da emenda [CC].', 'O texto deve ser aprovado [CC].']) {
      const gV = G.aplicarGates({ ...baseG, texto: corpo.replace('Falta a estimativa de impacto fiscal, que nenhum documento do processo traz [CC].', frase) });
      ok(gV.reprovacoes.some(r => r.gate === 'G10') && !G.rubricaMaquina({ ...baseG, texto: gV.texto, gates: gV }).itens.find(i => /^M11/.test(i.item)).ok, `recomendação NA CONCLUSÃO também reprova: "${frase.slice(0, 40)}"`);
    }
    const semMarcador = corpo.replace('Ao votar, decide-se instituir o marco nacional da negociação coletiva [CC]. Falta a estimativa de impacto fiscal, que nenhum documento do processo traz [CC].', 'Ao votar, decide-se instituir o marco nacional da negociação coletiva e falta a estimativa de impacto fiscal, que nenhum documento do processo traz.');
    ok(T.conferirRedacao(semMarcador, { tese: v.tese, catalogo: cat, ficha }).semEvidencia.some(x => /Conclusão/.test(x.secao)), 'parágrafo da conclusão sem identificador é apontado (a seção é conferida com o título novo)');
    ok(rC.itens.find(i => /^M13/.test(i.item)).ok && !G.rubricaMaquina({ ...baseG, texto: corpo.replace('Falta a estimativa de impacto fiscal, que nenhum documento do processo traz [CC].', 'x [CC].'), gates: gC }).itens.find(i => /^M13/.test(i.item)).ok, 'M13 exige a conclusão escrita com o que falta para decidir');
    // impressão
  ...
  const respostas = {
    ...
    tese: JSON.stringify({ afirmacoes: [
      ...
      conclusao: { id: 'CC', ponto_de_decisao: 'Decide-se manter ou não a delegação de alíquotas ao Ministro.', estabelecido: 'a regra passa de 20% para zero até US$ 50.', aberto: 'o efeito sobre a arrecadação não é verificável.', falta_para_decidir: 'a estimativa de renúncia, que o Executivo ainda não apresentou.', evidencias: ['P2', 'F1'] } }),
